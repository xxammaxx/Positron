...
async function main(): Promise<void> {
	...
	let specifyVersion = '';
	try {
		specifyVersion = execSync('specify --version 2>/dev/null', {
			encoding: 'utf-8',
			timeout: 5000,
		}).trim();
	} catch {
		/* not found locally — keine npx-fallback (supply-chain safety) */
	}
	results.push({
		name: 'SpecKit CLI (specify)',
		status: specifyVersion ? 'ok' : 'fail',
		detail: specifyVersion
			? `specify CLI gefunden: ${specifyVersion.slice(0, 100)}`
			: 'specify CLI nicht gefunden (lokal)',
	});

	// --- OpenCode ---
	let opencodeVersion = '';
	try {
		opencodeVersion = execSync('opencode --version 2>/dev/null', {
			encoding: 'utf-8',
			timeout: 5000,
		}).trim();
	} catch {
	...
	results.push({
		...
		detail: opencodeVersion
			? `opencode CLI gefunden: ${opencodeVersion.slice(0, 100)}`
			: 'opencode CLI nicht gefunden (lokal)',
	});
	...
	console.log('═══════════════════════════════════════');
	console.log(
		`   POSITRON_GITHUB_MODE = ${process.env['POSITRON_GITHUB_MODE'] ?? '(default: fake)'}`,
	);
	console.log('───────────────────────────────────────\n');
