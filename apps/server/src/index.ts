...
import type { ToolDefinition, GatewayConfig } from '@positron/tool-gateway';
	...
	return rows
		.filter((row) => {
			...
			try {
				...
				log.warn(
					`listRunsFromDb: Ungültige Phase "${String(row.phase)}" für Run ${String(row.id)} — wird übersprungen`,
				);
				return false;
			}
		})
...
function getDashboardMetrics() {
	...
	return {
		totalRuns: total,
		runsByStatus: { active, done, failed, blocked },
		runsByPhase: {},
	};
}
 ...
 * @throws {Error} Wenn der Body ungültig ist.
 */
function validateRepoRegistration(body: unknown): {
	owner: string;
	name: string;
} {
	if (!body || typeof body !== 'object') {
...
export function createApp(options: ServerOptions = {}) {
	...
	// ── Tool-Gateway: Registry + Gateway Service (read-only for monitoring) ──
	// Issue #224: Dashboard and Server integration for Tool Monitoring.
	// The gateway is DEFAULT DISABLED — no tool execution occurs through it.
	// These instances exist purely for read-only metadata exposure via API.
	const toolRegistry = new ToolRegistry();
	toolRegistry.register(repoReadFileDef, repoReadFileHandler);
	toolRegistry.register(repoListFilesDef, repoListFilesHandler);
	toolRegistry.register(repoGetDiffDef, repoGetDiffHandler);
	toolRegistry.register(testsDetectDef, testsDetectHandler);
	toolRegistry.register(testsRunSelectedDef, testsRunSelectedHandler);
	toolRegistry.register(evidenceAppendDef, evidenceAppendHandler);
	toolRegistry.register(githubReadIssueDef, githubReadIssueHandler);
	toolRegistry.register(githubCommentEvidenceDraftDef, githubCommentEvidenceDraftHandler);
	toolRegistry.seal();
	...
	app.get('/api/demo/blueprint/:runId', (req, res) => {
		...
		res.json({
			blueprint: (blueprintEvent.payload as Record<string, unknown>).blueprint,
	...
	app.post(
		...
		createDemoLiveRunHandler({
			...
			repository: {
				...repository,
				id: `${repository.owner}/${repository.repo}`,
			},
			runPipeline: (run: RunState) =>
				runFullPipeline(
					run,
					repository,
					activeWorkspaceAdapter,
	...
	app.get('/api/metrics', (_req, res) => {
		try {
			...
			const phaseDistribution = database
				.prepare(`
	...
	// Read-only endpoints for dashboard monitoring — no tool execution,
	// no runtime activation, no handler serialization.

	// GET /api/tool-gateway/status — Gateway status and feature flags
	app.get('/api/tool-gateway/status', (_req, res) => {
		const config = gatewayService.getConfig();
		const mcpExposeEnabled = process.env.POSITRON_MCP_EXPOSE_ENABLED === 'true';

		res.json({
			gatewayEnabled: config.enabled,
			mcpExposeEnabled,
			registeredTools: toolRegistry.size,
			sealed: toolRegistry.sealed,
			runtimeActive: false, // Gateway is always inactive when disabled
	...
	// GET /api/tool-gateway/tools — Registered tool metadata (read-only)
	app.get('/api/tool-gateway/tools', (_req, res) => {
		const tools = toolRegistry.list().map((def: ToolDefinition) => ({
			...
			allowedPhases: def.allowedPhases,
			egressPolicy: {
				allowedHosts: def.egressPolicy.allowedHosts,
				allowedPorts: def.egressPolicy.allowedPorts,
			},
			evidenceRequirements: {
				logArguments: def.evidenceRequirements.logArguments,
				logOutput: def.evidenceRequirements.logOutput,
				requireArtifact: def.evidenceRequirements.requireArtifact,
			},
	...
	app.post('/api/admin/runs/bulk-cancel', (_req, res) => {
		try {
			...
			const result = db
				.prepare(
					"UPDATE runs SET status = 'cancelled', finished_at = datetime('now') WHERE status IN ('active', 'blocked')",
				)
				.run();
	...
	app.post('/api/admin/runs/bulk-retry', (_req, res) => {
		try {
			...
			const result = db
				.prepare(
					"UPDATE runs SET status = 'active', phase = 'QUEUED' WHERE status IN ('failed', 'blocked')",
				)
				.run();
