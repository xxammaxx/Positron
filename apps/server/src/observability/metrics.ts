...
export function classifyGitHubError(err: Error): string {
	...
	if (msg.includes('rate limit') || msg.includes('secondary rate limit')) return 'rate_limit';
	if (msg.includes('authentication') || msg.includes('bad credentials')) return 'auth';
 ...
 */
export function recordGitHubApiSuccess(method: GitHubApiMethod, durationMs: number): void {
...
export function recordGitHubApiFailure(
	...
	if (errorKind === 'rate_limit') {
		const rateType = err.message.toLowerCase().includes('secondary') ? 'secondary' : 'primary';
...
export function classifyRuntimeError(err: Error): string {
	...
	// Non-zero exit
	if (msg.includes('exit code') || msg.includes('exited with')) return 'nonzero_exit';
	// Missing binary
	if (msg.includes('not found or not executable') || msg.includes('enoent'))
		return 'missing_binary';
	// Invalid output
	if (msg.includes('invalid') || msg.includes('parse error') || msg.includes('unexpected token'))
