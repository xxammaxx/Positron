...
import { createRunId, buildRemoteUrl, MAX_FIX_LOOPS, generateBranchName, parsePhase, parseRunStatus } from '@positron/shared';
import { createRun, transition, markFailed, tryTransitionWithGates, registerGateEvaluator, registerWorkspaceCleanup, runCleanup } from '@positron/run-state';
import { TestCommandDetector, TestRunner } from '@positron/sandbox';
import { GitHubStatusSyncService } from '@positron/github-adapter';
import type {
  Phase, EventLevel, RepositoryConfig,
  SpecKitAdapter, OpenCodeAdapter,
} from '@positron/shared';
import type { RunState, RunEventData } from '@positron/run-state';
import type { GitHubAdapter, GitHubStatusSyncInput, GitHubStatusSyncResult, EvidenceItem } from '@positron/github-adapter';
...
function saveArtifact(runId: string, kind: string, content: string | string[], deps: PipelineDeps): void {
  try {
...
async function generateResearchDocument(
  ...
  try {
    ...
    console.warn(`[Worker] generateResearchDocument: Failed to fetch issue #${issueNumber}: ${String(err).slice(0, 200)}`);
  }
  ...
  if (researchApiKey) {
    try {
      ...
      console.warn(`[Worker] generateResearchDocument: Brave Search failed: ${String(err).slice(0, 200)}`);
    }
  }
...
/**
 * Registers all 6 GateType evaluators for the worker pipeline.
 * Fake/Dry-Run: evaluators pass non-blocking. Real mode: enforce based on state.
 */
function initializeGateEvaluators(): void {
	const isRealMode = process.env.POSITRON_ENABLE_REAL_RUN === 'true';

	registerGateEvaluator('pre_write', async () => {
		if (!isRealMode) return { gate: 'pre_write', passed: true, blocking: false, reason: 'Fake/dry-run: pre_write skipped', evaluatedAt: new Date().toISOString() };
		return { gate: 'pre_write', passed: true, blocking: false, reason: 'Pre-write gate passed', evaluatedAt: new Date().toISOString() };
	});

	registerGateEvaluator('evidence_required', async () => {
		if (!isRealMode) return { gate: 'evidence_required', passed: true, blocking: false, reason: 'Fake/dry-run: evidence skipped', evaluatedAt: new Date().toISOString() };
		return { gate: 'evidence_required', passed: true, blocking: false, reason: 'Evidence gate passed', evaluatedAt: new Date().toISOString() };
	});

	registerGateEvaluator('pre_pr', async () => {
		if (!isRealMode) return { gate: 'pre_pr', passed: true, blocking: false, reason: 'Fake/dry-run: pre_pr skipped', evaluatedAt: new Date().toISOString() };
		return { gate: 'pre_pr', passed: true, blocking: false, reason: 'PR gate passed', evaluatedAt: new Date().toISOString() };
	});

	registerGateEvaluator('pre_merge', async () => {
		if (!isRealMode) return { gate: 'pre_merge', passed: true, blocking: false, reason: 'Fake/dry-run: pre_merge skipped', evaluatedAt: new Date().toISOString() };
		const mk = process.env.POSITRON_MERGE_KILL_SWITCH !== 'false';
		const ma = process.env.POSITRON_ENABLE_MERGE === 'true';
		if (mk) return { gate: 'pre_merge', passed: false, blocking: true, reason: 'POSITRON_MERGE_KILL_SWITCH active', evaluatedAt: new Date().toISOString() };
		if (!ma) return { gate: 'pre_merge', passed: false, blocking: true, reason: 'POSITRON_ENABLE_MERGE not set', evaluatedAt: new Date().toISOString() };
		return { gate: 'pre_merge', passed: true, blocking: false, reason: 'Merge gate passed', evaluatedAt: new Date().toISOString() };
	});

	registerGateEvaluator('security', async (_gate, run) => {
		if (!isRealMode) return { gate: 'security', passed: true, blocking: false, reason: 'Fake/dry-run: security skipped', evaluatedAt: new Date().toISOString() };
		if (run.status === 'failed' && run.lastError) return { gate: 'security', passed: false, blocking: true, reason: `Security blocked: ${run.lastError.slice(0, 100)}`, evaluatedAt: new Date().toISOString() };
		if (run.phase === 'FAILED_UNSAFE') return { gate: 'security', passed: false, blocking: true, reason: 'Security blocked: FAILED_UNSAFE', evaluatedAt: new Date().toISOString() };
		return { gate: 'security', passed: true, blocking: false, reason: 'Security: no issues', evaluatedAt: new Date().toISOString() };
	});

	registerGateEvaluator('human_approval', async () => {
		if (!isRealMode) return { gate: 'human_approval', passed: true, blocking: false, reason: 'Fake/dry-run: human_approval skipped', evaluatedAt: new Date().toISOString() };
		return { gate: 'human_approval', passed: false, blocking: true, reason: 'Human approval required for merge', evaluatedAt: new Date().toISOString() };
	});
}
...
async function executePhase(
  ...
  switch (current.phase) {
    ...
    case 'CLAIMED':
      if (deps.syncService) {
        const syncInput: GitHubStatusSyncInput = {
          runId: current.id, owner: deps.repository.owner, repo: deps.repository.repo,
          issueNumber: current.issueNumber, phase: 'CLAIMED', status: 'active',
          branchName: current.branch ?? undefined,
        };
        await safeSync(deps.syncService, () => deps.syncService!.syncRunAccepted(syncInput), current.id, 'CLAIMED', deps);
      }
    ...
    case 'REPO_SYNC':
      try {
        const workspaceRepository = {
          ...
          remoteUrl: deps.repository.remoteUrl ?? buildRemoteUrl(deps.repository.owner, deps.repository.repo),
        };
    ...
    case 'ISSUE_CONTEXT':
      ...
      break;
    case 'WEB_RESEARCH': {
      const researchDoc = await generateResearchDocument(deps.github, deps.repository, current.issueNumber);
      ...
      if (realSpeckit) {
        try {
          ...
          if (initResult.status === 'success') {
            storeEvent({ id: createRunId(), runId: current.id, phase: 'SPECIFY', level: 'INFO', message: `Spec Kit initialized: ${initResult.summary}`, payload: null, createdAt: new Date().toISOString() }, deps);
          ...
          storeEvent({ id: createRunId(), runId: current.id, phase: 'SPECIFY', level: 'WARN', message: `Real Spec Kit error: ${String(err).slice(0, 200)}`, payload: null, createdAt: new Date().toISOString() }, deps);
        }
      }
    ...
    case 'PLAN': {
      ...
      if (realSpeckit) {
        try {
          ...
          storeEvent({ id: createRunId(), runId: current.id, phase: 'PLAN', level: 'WARN', message: `Real Spec Kit error: ${String(err).slice(0, 200)}`, payload: null, createdAt: new Date().toISOString() }, deps);
        }
      }
    ...
    case 'TASKS': {
      ...
      if (realSpeckit) {
        try {
          ...
          storeEvent({ id: createRunId(), runId: current.id, phase: 'TASKS', level: 'WARN', message: `Real Spec Kit error: ${String(err).slice(0, 200)}`, payload: null, createdAt: new Date().toISOString() }, deps);
        }
      }
    ...
    case 'ANALYZE': {
      ...
      try {
        ...
        storeEvent({ id: createRunId(), runId: current.id, phase: 'ANALYZE', level: 'WARN', message: `Analyze error: ${String(err).slice(0, 200)}`, payload: null, createdAt: new Date().toISOString() }, deps);
    ...
    case 'REVIEW': {
      ...
      const existingKinds = new Set(
        (getDb(deps).prepare('SELECT DISTINCT kind FROM artifacts WHERE run_id = ?').all(current.id) as Array<{ kind: string }>)
          .map(r => r.kind),
      );
    ...
    case 'IMPLEMENT': {
      ...
      try {
        ...
        if (ir.status === 'blocked') {
          storeEvent({ id: createRunId(), runId: current.id, phase: 'IMPLEMENT', level: 'WARN', message: `Implement blocked: ${ir.blockedReason ?? 'policy'}`, payload: { result: ir }, createdAt: new Date().toISOString() }, deps);
        }
        ...
        if (detection.commands.length === 0) {
          ...
          if (deps.syncService && report) {
            ...
            if (report.status === 'blocked') {
              await safeSync(deps.syncService, () => deps.syncService!.syncBlocked({
                ...syncInput, error: { type: 'blocked', message: report.summary },
              }), current.id, 'TEST', deps);
            } else if (report.status === 'failed') {
              await safeSync(deps.syncService, () => deps.syncService!.syncTestReport(syncInput), current.id, 'TEST', deps);
            } else {
              await safeSync(deps.syncService, () => deps.syncService!.syncTestReport(syncInput), current.id, 'TEST', deps);
            }
          }
    ...
    case 'COMMIT': {
      ...
      try {
        ...
        result = await tryTransitionWithGates(current, 'PR_CREATE', `Commit skipped: ${String(err).slice(0, 100)}`, 'WARN');
      }
      break;
    }
    case 'PR_CREATE': {
      ...
      try {
        const pr = await deps.github.createPullRequest({
          owner: deps.repository.owner, repo: deps.repository.repo,
        ...
        if (deps.syncService) {
          ...
          await safeSync(deps.syncService, () => deps.syncService!.syncPrCreated(syncInput), current.id, 'PR_CREATE', deps);
        }
    ...
    case 'MERGE': {
      ...
      try {
        const prs = await deps.github.listPullRequests({
          owner: deps.repository.owner, repo: deps.repository.repo,
          head: `${deps.repository.owner}:${branch}`, state: 'open',
        });
        pr = prs[0] ?? null;
      ...
      if (pr.state !== 'open') {
        result = await tryTransitionWithGates(current, 'DONE', `PR #${pr.number} ist ${pr.state} — Merge übersprungen`, 'WARN');
        break;
      }
      ...
      if (mergeDryRun) {
        ...
        for (let retry = 0; retry <= maxMergeableRetries; retry++) {
          try {
            const prDetail = await deps.github.getPullRequest(deps.repository.owner, deps.repository.repo, pr.number);
        ...
        const testEvent = getEvents(current.id, deps).find(e => e.phase === 'TEST' && e.level === 'INFO');
        ...
        storeEvent({
          id: createRunId(), runId: current.id, phase: 'MERGE', level: 'GATE' as EventLevel,
        ...
        try {
          ...
          await deps.github.createIssueComment(
            { owner: deps.repository.owner, repo: deps.repository.repo, issueNumber: current.issueNumber },
        ...
        result = await tryTransitionWithGates(current, 'DONE',
      ...
      if (mergeKillSwitch) {
        result = await tryTransitionWithGates(current, 'DONE', 'Merge BLOCKED: Kill-Switch (POSITRON_MERGE_KILL_SWITCH=true)', 'WARN');
        break;
      }
      if (!mergeAllowed) {
        result = await tryTransitionWithGates(current, 'DONE', 'Merge skipped (POSITRON_ENABLE_MERGE not set)', 'INFO');
        break;
      }
      if (current.status !== 'active') {
        result = await tryTransitionWithGates(current, 'DONE', `Merge blocked: Run status is ${current.status}`, 'WARN');
        break;
      }
      ...
      try {
        ...
        if (mergeResult.merged) {
          if (deps.syncService) {
            ...
            await safeSync(deps.syncService, () => deps.syncService!.syncMerged(syncInput), current.id, 'MERGE', deps);
          }
          try {
            await deps.github.closeIssue(deps.repository.owner, deps.repository.repo, current.issueNumber);
          } catch { /* best-effort */ }
          result = await tryTransitionWithGates(current, 'DONE', `PR #${pr.number} merged: ${mergeResult.sha?.slice(0, 7)}`, 'INFO');
        } else {
          result = await tryTransitionWithGates(current, 'DONE', `PR #${pr.number} not mergeable: ${mergeResult.message ?? 'unknown'}`, 'WARN');
        }
      } catch (err) {
        storeEvent({
...
export async function runPipeline(
  ...
  registerWorkspaceCleanup(async (workspacePath: string, _runId: string) => {
    const result = await deps.workspace.destroyWorkspace(workspacePath);
  ...
  for (let i = 0; i < maxSteps; i++) {
    ...
    if (sig?.toLowerCase() === 'abort') {
      const cancelled = { ...current, status: 'cancelled' as RunState['status'], finishedAt: new Date().toISOString() };
      storeEvent({ id: createRunId(), runId: current.id, phase: current.phase, level: 'INFO', message: 'Run aborted by user (worker)', payload: { action: 'abort' }, createdAt: new Date().toISOString() }, deps);
      saveRunToDb(cancelled, deps);
      return cancelled;
    }
    if (sig?.toLowerCase() === 'pause') {
      // Worker pause: wait and re-check signal
      storeEvent({ id: createRunId(), runId: current.id, phase: current.phase, level: 'INFO', message: 'Run paused by user (worker — waiting)', payload: null, createdAt: new Date().toISOString() }, deps);
      while (true) {
        await new Promise(r => setTimeout(r, 3000));
        const s = checkSignal(deps.db, current.id, current.phase);
        if (s?.toLowerCase() === 'abort') {
          const cancelled = { ...current, status: 'cancelled' as RunState['status'], finishedAt: new Date().toISOString() };
          storeEvent({ id: createRunId(), runId: current.id, phase: current.phase, level: 'INFO', message: 'Run aborted while paused (worker)', payload: null, createdAt: new Date().toISOString() }, deps);
          saveRunToDb(cancelled, deps);
          return cancelled;
        }
    ...
    if (next.phase === current.phase || next.phase === 'DONE' || next.phase.startsWith('FAILED')) {
      ...
      if (fixLoopEnabled && next.phase === 'FAILED_TRANSIENT' && attempt < maxAttempts) {
        ...
        const allTransient = getEvents(next.id, deps).filter((e: RunEventData) => e.phase === 'FAILED_TRANSIENT');
      ...
      if (deps.syncService) {
        const syncInput: GitHubStatusSyncInput = {
          runId: next.id, owner: deps.repository.owner, repo: deps.repository.repo,
      ...
      if (next.workspacePath) {
        ...
        storeEvent({
          id: createRunId(), runId: next.id, phase: next.phase, level: 'INFO',
          message: cleanupResult.cleaned ? `Workspace cleaned: ${next.workspacePath}` : `Workspace cleanup skipped: ${cleanupResult.reason ?? 'no reason'}`,
          payload: { ...cleanupResult }, createdAt: new Date().toISOString(),
        }, deps);
      }
      return next;
    }
  ...
  if (deps.syncService) {
    const syncInput: GitHubStatusSyncInput = {
      runId: result.run.id, owner: deps.repository.owner, repo: deps.repository.repo,
    ...
    await safeSync(deps.syncService, () => deps.syncService!.syncBlocked(syncInput), result.run.id, 'FAILED_BLOCKED', deps);
  }
