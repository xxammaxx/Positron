...
describe('PhasePipeline', () => {
  ...
  test('marks completed phases as completed', () => {
    const { container } = render(<PhasePipeline completedPhases={['QUEUED', 'CLAIMED', 'REPO_SYNC']} />);
  ...
  test('onPhaseClick handler is called with phase name', () => {
    ...
    const { container } = render(<PhasePipeline currentPhase="TEST" completedPhases={['QUEUED', 'CLAIMED']} onPhaseClick={handleClick} />);
    const phaseButtons = container.querySelectorAll('button');
    // Click the TEST button (current phase)
    const testButton = Array.from(phaseButtons).find(
      b => b.getAttribute('aria-label')?.includes('TEST')
    );
    if (testButton) {
      testButton.click();
