        ...
        {recent.map(run => (
          ...
          >
            <div className={`w-2 h-2 rounded-full ${
              run.status === 'active' ? 'bg-sky-500 animate-pulse' :
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-500">
                {run.phase ?? 'unknown'} · {run.startedAt ? new Date(run.startedAt).toLocaleString() : 'recently'}
              </p>
            </div>
            <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${STATUS_BADGE[run.status] ?? ''}`}>
