...
export default function EvidencePage(): React.ReactElement {
  ...
  const [runs, setRuns] = useState<Array<{ id: string; issueNumber: number }>>([]);
  ...
  const fetchEvidence = useCallback(async () => {
    try {
      ...
      else if (evidenceData.summary) { setSummary(evidenceData.summary); setEvidence([]); }
    ...
    return evidence.filter(e => {
      ...
      if (searchTerm) {
        ...
        if (!e.summary.toLowerCase().includes(q) &&
            ...
            !(KIND_LABELS[e.kind] ?? '').toLowerCase().includes(q)) return false;
      }
      return true;
    });
    ...
    <div>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          <div className="card !border-l-green-400 !border-l-4 !bg-green-500/5">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Artifacts</p>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{summary.totalArtifacts as number}</p>
          ...
          <div className="card !border-l-sky-400 !border-l-4 !bg-sky-500/5">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Test Events</p>
            <p className="text-2xl font-bold text-sky-600 dark:text-sky-400">{summary.testEvents as number}</p>
          </div>
          <div className="card !border-l-red-400 !border-l-4 !bg-red-500/5">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Errors</p>
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{summary.errorEvents as number}</p>
          </div>
          <div className="card !border-l-amber-400 !border-l-4 !bg-amber-500/5">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Warnings</p>
            <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{summary.warningEvents as number}</p>
          </div>
        </div>
      )}
      ...
      {summary && evidence.length === 0 && (
        <div className="card text-center py-8 mt-4">
          ...
          <p className="text-sm text-slate-800 dark:text-slate-300 font-medium mb-1">Evidence Summary Available</p>
      ...
      {evidence.length > 0 && filtered.length === 0 && (
        <div className="card text-center py-8">
          ...
          <p className="text-sm text-slate-800 dark:text-slate-300 font-medium mb-1">No matching evidence</p>
      ...
      {evidence.length > 0 && filtered.length > 0 && (
        <>
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            />
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="input max-w-[130px]">
              <option value="all">All Status</option>
              <option value="pass">Pass</option>
              <option value="fail">Fail</option>
              <option value="partial">Partial</option>
            </select>
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="input max-w-[130px]">
              {types.map(t => (
                <option key={t} value={t}>{t === 'all' ? 'All Types' : t}</option>
              ))}
            </select>
            <select value={runFilter} onChange={e => setRunFilter(e.target.value)} className="input max-w-[180px]">
              <option value="all">All Runs</option>
              {runOptions.filter(r => r !== 'all').map(r => (
                <option key={r} value={r}>Run {r.slice(0, 8)}</option>
              ))}
            </select>
                    ...
                    <React.Fragment key={item.id}>
                      ...
                      >
                        <td className="px-4 py-2.5">
                          <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${KIND_COLORS[item.kind] ?? 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-700'}`}>
                            {KIND_LABELS[item.kind] ?? item.kind}
                          </span>
                        </td>
                        <td className="px-4 py-2.5">
                          <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                            item.status === 'pass' ? 'bg-green-100 dark:bg-green-500/15 text-green-700 dark:text-green-400' :
                            item.status === 'fail' ? 'bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-400' :
                        ...
                        <tr className="bg-slate-50 dark:bg-slate-800/20">
                          <td colSpan={5} className="px-4 py-3">
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              <div>
                                <span className="text-slate-500 dark:text-slate-500 block">Kind</span>
                                <span className="text-slate-700 dark:text-slate-300 font-mono">{item.kind}</span>
                              </div>
                              <div>
                                <span className="text-slate-500 dark:text-slate-500 block">Phase</span>
                                <span className="text-slate-700 dark:text-slate-300">{item.runPhase ?? '-'}</span>
                              </div>
                              <div>
                                <span className="text-slate-500 dark:text-slate-500 block">Source</span>
                                <span className="text-slate-700 dark:text-slate-300">{item.source}</span>
                              </div>
                              <div>
                                <span className="text-slate-500 dark:text-slate-500 block">Timestamp</span>
                                <span className="text-slate-700 dark:text-slate-300">{new Date(item.timestamp).toLocaleString()}</span>
                              </div>
                            </div>
                          </td>
                        </tr>
