...
const phaseStatusColors: Record<PhaseStatus, { bg: string; border: string; text: string; icon: string }> = {
  ...
  completed:  { bg: 'bg-green-500/10', border: 'border-green-700', text: 'text-green-400',     icon: '✓' },
  ...
  human:      { bg: 'bg-purple-500/10', border: 'border-purple-800', text: 'text-purple-400',  icon: '👤' },
};

/** Kürzel für Phasen-Namen (28 Phasen) */
const PHASE_SHORT: Record<Phase, string> = {
  QUEUED: 'Q', CLAIMED: 'C', REPO_SYNC: 'RS', ISSUE_CONTEXT: 'IC',
  WEB_RESEARCH: 'WR', SPECIFY: 'SP', CLARIFY_OPTIONAL: 'CO', PLAN: 'PL',
  TASKS: 'TK', ANALYZE: 'AN', REVIEW: 'RV', IMPLEMENT: 'IM',
  TEST: 'TE', VERIFY: 'VE', COMMIT: 'CM', PR_CREATE: 'PR',
  MERGE: 'MG', DONE: 'OK',
  FAILED: 'FL', FAILED_TRANSIENT: 'FT', FAILED_BLOCKED: 'FB', FAILED_UNSAFE: 'FU',
  BLOCKED_PUSH: 'BP', BLOCKED_MERGE: 'BM', GATE_APPROVE: 'GA', GATE_REVISE: 'GR',
  RESUME_PENDING: 'RP', CLEANUP: 'CL',
};

function PhaseNode({ phase, status, onClick }: { phase: Phase; status: PhaseStatus; onClick?: (p: Phase) => void }) {
  ...
  return (
    <div className="card panel-stagger">
          ...
          {mainPhases.map((phase, i) => (
            <React.Fragment key={phase}>
              ...
              {i < mainPhases.length - 1 && (
                <div className={`w-2 h-px mt-7 shrink-0 ${completedPhases.includes(phase) ? 'bg-green-400' : 'bg-slate-700'}`} />
              )}
            </React.Fragment>
          ))}
        </ol>
        ...
        <div className="mt-4 pt-4 border-t border-slate-800">
          ...
          <div className="flex items-start gap-1 flex-wrap">
            {errorPhases.map((phase) => {
              ...
              return (
                <PhaseNode key={phase} phase={phase} status={status} onClick={onPhaseClick} />
              );
            })}
          </div>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 text-[10px] text-slate-500 font-['IBM_Plex_Sans']">
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-600" /> Pending</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-sky-400" /> Current</span>
        ...
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400" /> Blocked</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-400" /> Human</span>
      </div>
    </div>
  );
}
