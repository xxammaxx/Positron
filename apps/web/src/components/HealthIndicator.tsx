  ...
  useEffect(() => {
    ...
    async function check(): Promise<void> {
      try {
        ...
        setTooltip([`Uptime: ${h.uptime}s`, modeLine, '', adapterStatus].filter(Boolean).join('\n'));
      } catch {
        if (cancelled) return;
        setHealth(null);
        setStatus('error');
    ...
    <div className="flex items-center gap-3">
      ...
      {health?.mode && (
        <div
          ...
          title={isFakeMode ? 'Demo-Mode: Alle Adapter sind Fake. Setze POSITRON_GITHUB_MODE=real für echten Betrieb.' : 'Live-Mode: Echte Adapter aktiv'}
        >
          <span className={`w-1.5 h-1.5 rounded-full ${isFakeMode ? 'bg-amber-400' : 'bg-emerald-400'}`} />
