...
const PHASES = [
  'all', 'ingest', 'specify', 'plan', 'tasks', 'implement',
  'review', 'fix', 'done', 'failed', 'cancelled',
] as const;
...
export default function RunsPage(): React.ReactElement {
    ...
    return runs.filter(r => {
      ...
      if (searchTerm) {
        ...
        if (!r.id.toLowerCase().includes(q) &&
            !(r.issueNumber?.toString() ?? '').includes(q)) return false;
      }
      return true;
    });
  ...
  const statusCounts = useMemo(() => {
    const counts: Record<string, number> = { active: 0, done: 0, failed: 0, blocked: 0, cancelled: 0 };
    ...
    return (
      <div>
        ...
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mt-4">
          ...
          <button onClick={fetchRuns} className="text-xs mt-2 text-red-600 dark:text-red-400 hover:underline">
            Retry
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        />
        <select value={phaseFilter} onChange={e => setPhaseFilter(e.target.value)} className="input max-w-[140px]">
          {PHASES.map(p => (
      ...
      {runs.length > 0 && filtered.length === 0 && (
        <div className="card text-center py-8">
          ...
          <p className="text-sm text-slate-800 dark:text-slate-300 font-medium mb-1">No matching runs</p>
              ...
              <tbody className="divide-y divide-slate-200/50 dark:divide-slate-800/50">
                {filtered.map(run => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${STATUS_COLORS[run.status] ?? ''}`}>
