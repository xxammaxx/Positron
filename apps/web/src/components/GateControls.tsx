...
export default function GateControls({
  ...
  if (!GATE_PHASES.has(currentPhase)) {
    return <div className="card text-center text-slate-500 text-sm py-6">Keine Genehmigung erforderlich</div>;
  }
  ...
  return (
    <div className="card">
      <h3 className="text-sm font-medium text-amber-400 mb-2">
        ⚡ Gate-Entscheidung erforderlich
      </h3>
          ...
          {showReviseForm && (
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-slate-500">
                {reviseReason.trim().length}/20 Zeichen
              </p>
            </div>
          )}
        </div>
