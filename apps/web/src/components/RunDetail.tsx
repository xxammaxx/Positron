...
export default function RunDetail(): React.ReactElement {
  ...
  const { events, evidence, isConnected, runStatus: sseRunStatus, error: sseError } = useSSE(id ?? null);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-slate-400 mb-4">
        ...
        <span>/</span>
        <Link to="/runs" className="hover:text-blue-400 transition-colors">Runs</Link>
        <span>/</span>
      ...
      {(errorBanner || run.lastError || cancelError) && (
        <div className="mb-4 p-3 bg-red-900/50 border border-red-700 rounded-lg text-red-200 text-sm flex items-center justify-between">
          ...
          <button
            onClick={() => { setErrorBanner(null); setCancelError(null); }}
      ...
      <div className="card mb-6">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                ...
                isCancelled ? 'bg-amber-600/20 text-amber-400' :
                effectiveStatus === 'done' ? 'bg-green-600/20 text-green-400' :
                effectiveStatus === 'failed' ? 'bg-red-600/20 text-red-400' :
            ...
            <div className="space-y-1 text-sm text-slate-400">
              ...
              <p>
                ...
                {isRunning ? <span className="text-blue-400 animate-pulse">running ({formatDuration(duration)})</span> : formatDuration(duration)}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Copy Run ID */}
            <button onClick={handleCopyRunId} className="btn-ghost text-xs py-1 px-2" title="Copy Run ID">
