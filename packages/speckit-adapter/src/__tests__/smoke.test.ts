...
describe('FakeSpecKitAdapter', () => {
  ...
  test('runSpecify in artifact-only mode', async () => {
    const adapter = new FakeSpecKitAdapter();
    const result = await adapter.runSpecify({ runId: 'test', workspacePath: '/tmp', issueTitle: 'test', mode: 'artifact-only' });
    expect(result.status).toBe('skipped');
  });

  test('runSpecify in safe-cli mode', async () => {
    const adapter = new FakeSpecKitAdapter();
    const result = await adapter.runSpecify({ runId: 'test', workspacePath: '/tmp', issueTitle: 'test', mode: 'safe-cli' });
