...
import { SpecKitNotInstalledError, SpecKitTimeoutError, SpecKitCommandFailedError, SpecKitWorkspaceInvalidError, SpecKitCommandNotAllowedError, SpecKitUnsupportedCommandError } from '@positron/shared';
...
export class RealSpecKitAdapter implements SpecKitAdapter {
  ...
  async healthCheck(workspacePath: string): Promise<SpecKitHealth> {
    try {
      ...
      return { available: false, reason: `specify version exited with code ${String(result.exitCode)}` };
    } catch {
  ...
  async initialize(input: SpecKitRunInput): Promise<SpecKitCommandResult> {
    ...
    try {
      const result = await runCommand('specify', ['init', '--integration', input.aiAgent ?? 'generic'], {
   ...
   */
  private agentSlashCommandResult(
    phase: string,
