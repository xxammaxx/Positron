...
const envValueArb = fc.oneof(
	...
	fc.constant(
		"sk-fake-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
	),
);

/** Arbitrary: complete KEY=VALUE line */
const validLineArb = fc
	.tuple(validKeyArb, envValueArb)
	.map(([k, v]) => `${k}=${v}`);

/** Arbitrary: KEY with spaces = value */
const spaceKeyLineArb = fc
	.tuple(anyKeyArb, envValueArb)
	.map(([k, v]) => `${k}=${v}`);
		...
		fc.assert(
			fc.property(validKeyArb, (key) => {
				const tmpDir = fs.mkdtempSync(
					path.join(os.tmpdir(), "positron-docker-"),
				);
				try {
					const provider = new DockerSecretProvider(tmpDir);
					expect(provider.getSecret(key)).toBeNull();
