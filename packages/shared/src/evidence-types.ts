...
export function validateContextManifest(manifest: ContextManifest): string[] {
	...
	if (!manifest.constraints?.constitution)
		errors.push('constraints.constitution must be non-empty');
	...
	if (manifest.context?.recentChanges && manifest.context.recentChanges.length > 0) {
...
export function validateEvidenceLog(log: EvidenceLog): string[] {
	...
	for (const gate of log.gates) {
		if (
			...
			errors.push(`Gate "${gate.name}" claims 'pass' but has no evidence`);
		}
		...
		if (
			...
			if (typeof ev.passed === 'number' && typeof ev.total === 'number' && ev.passed < ev.total) {
				errors.push(
...
export interface ReviewReport {
	 ...
	 * evidenceIntegrity.
	 */
	checklistResults: Record<string, 'pass' | 'fail' | 'partial' | 'warning' | 'not_applicable'>;
...
export function validateReviewReport(report: ReviewReport): string[] {
	...
	if (report.verdict === 'pass' && report.blockingFindings.length > 0) {
		errors.push('Reviewer cannot give "pass" verdict when blockingFindings is non-empty');
	}

	// -- Verdict: 'pass' erfordert evidenceChecked --
	if (report.verdict === 'pass' && report.evidenceChecked.length === 0) {
		errors.push('Reviewer cannot give "pass" verdict when evidenceChecked is empty');
	}

	// -- Verdict: 'pass' darf kein missingEvidence haben (ausser optional) --
	if (report.verdict === 'pass' && report.missingEvidence.length > 0) {
		// Da wir nicht wissen können, welche optional sind, ist non-empty immer ein Problem
		errors.push('Reviewer cannot give "pass" verdict when missingEvidence is non-empty');
	}
	...
	if (report.verdict === 'fail' && !report.humanApprovalRequired) {
		errors.push('When verdict is "fail", humanApprovalRequired must be true');
	}
 ...
 */
export function validateVerificationContract(contract: VerificationContract): string[] {
	const errors: string[] = [];
	...
	if (!contract.evidenceRequirements || Object.keys(contract.evidenceRequirements).length === 0) {
