...
function createMinimalDeclaration(overrides: Partial<AgentDeclaration> = {}): AgentDeclaration {
	return {
		name: 'test-agent',
		type: 'cli',
		version: '1.0.0',
...
function createMockAdapter(overrides: Partial<AgentDeclaration> = {}): CodingAgentAdapter {
...
describe('validateAgentDeclaration', () => {
	...
	it('rejects declaration with invalid riskLevel', () => {
		...
		const errors = validateAgentDeclaration(decl);
		expect(errors).toContain('riskLevel must be one of: low, medium, high, critical');
	});
	...
	it('rejects declaration with invalid type', () => {
		...
		const errors = validateAgentDeclaration(decl);
		expect(errors).toContain('type must be one of: cli, api, ide, service, human');
	});
...
describe('AgentCapabilityRegistry', () => {
	...
	describe('findAgentsForCapabilities', () => {
		it('returns matching agents', () => {
			...
			const result = registry.findAgentsForCapabilities(['repo_read', 'code_write']);
		...
		it('returns empty when no match', () => {
			...
			const result = registry.findAgentsForCapabilities(['code_write', 'test_run']);
...
describe('CodingAgentAdapter contract', () => {
	...
	it('runPhase result must contain expected fields', async () => {
		...
		expect(['success', 'failed', 'blocked', 'skipped']).toContain(result.status);
