...
describe('resumeFromEvents', () => {
  test('setzt korrekt aus Events fort', () => {
    const events: RunEventData[] = [
      { id: 'e1', runId: 'run-1', phase: 'CLAIMED', level: 'INFO', message: 'claimed', payload: null, createdAt: new Date().toISOString() },
      { id: 'e2', runId: 'run-1', phase: 'REPO_SYNC', level: 'INFO', message: 'synced', payload: null, createdAt: new Date().toISOString() },
      { id: 'e3', runId: 'run-1', phase: 'SPECIFY', level: 'INFO', message: 'spec done', payload: null, createdAt: new Date().toISOString() },
    ];
