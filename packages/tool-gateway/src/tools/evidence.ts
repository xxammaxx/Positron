...
export const evidenceAppendHandler: ToolHandler = async (call): Promise<ToolResult> => {
	try {
		...
		// Build full evidence item for persistence.
		// Included in the output so the GatewayService's onEvidence callback
		// and downstream consumers can access the complete evidence record.
		...
		return {
			...
			output: {
				...
				timestamp,
				summary,
				evidenceItem, // Full evidence record for persistence
			},
