...
export const testsDetectHandler: ToolHandler = async (
  call,
): Promise<ToolResult> => {
  try {
    ...
    try {
      const pkgJson = execSync("node -e \"console.log(JSON.stringify(require('./package.json').scripts || {}))\"", {
...
export const testsRunSelectedHandler: ToolHandler = async (
  call,
): Promise<ToolResult> => {
  ...
  try {
    ...
    if (!validateTestCommand(command)) {
      return {
        ...
        blockedReason:
          "SCHEMA_VALIDATION_FAILED: command not in allowed test prefixes",
      };
    }
    ...
    for (const char of dangerousChars) {
      if (command.includes(char)) {
        return {
          ...
          blockedReason:
            "SCHEMA_VALIDATION_FAILED: shell metacharacters not allowed",
        };
      }
    }
