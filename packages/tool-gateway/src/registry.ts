...
export class ToolRegistry {
  ...
  register(definition: ToolDefinition, handler: ToolHandler): void {
    ...
    if (!definition.inputSchema || typeof definition.inputSchema !== "object") {
      throw new Error(
        `Tool "${definition.id}" must have a valid inputSchema`,
      );
    }
  ...
  listForPhase(phase: Phase): ToolDefinition[] {
    return this.list().filter((def) =>
      def.allowedPhases.includes(phase),
    );
  }
  ...
  private validateToolId(id: string): void {
    if (typeof id !== "string" || id.length < 3) {
      throw new Error(
        `Tool ID must be a string with at least 3 characters, got: "${id}"`,
      );
    }
    ...
    if (!/^[\x00-\x7F]+$/.test(id)) {
      throw new Error(
        `Tool ID must be ASCII-only, got: "${id}"`,
      );
    }
  ...
  private validateRiskLevel(level: string): void {
    ...
    if (!validLevels.includes(level)) {
      throw new Error(
        `Invalid risk level: "${level}". Must be one of: ${validLevels.join(", ")}`,
      );
    }
  }
}
