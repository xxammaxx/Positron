...
export class GitHubStatusSyncService {
	...
	private async syncLabels(
		...
		for (const label of lifecycle.add) {
			try {
				await this.adapter.addIssueLabels(ref, [label]);
				added.push(label);
			} catch {
				/* toleriere */
			}
		...
		for (const label of lifecycle.remove) {
			try {
				await this.adapter.removeIssueLabel(ref, label);
				removed.push(label);
			} catch {
				/* toleriere 404 */
			}
	...
	async syncRunAccepted(input: GitHubStatusSyncInput): Promise<GitHubStatusSyncResult> {
		try {
			...
			return {
				...
				labelsAdded: [],
				labelsRemoved: [],
				reason: redactSecrets(String(err)),
			};
		}
	}
	...
	async syncPhaseUpdate(input: GitHubStatusSyncInput): Promise<GitHubStatusSyncResult> {
		try {
			const comment = renderSyncPhaseUpdate(
				input.runId,
				input.phase,
				input.status,
				input.message ?? '',
	...
	async syncTestReport(input: GitHubStatusSyncInput): Promise<GitHubStatusSyncResult> {
		try {
			...
			if (!report)
				return { status: 'failed', labelsAdded: [], labelsRemoved: [], reason: 'Kein TestReport' };
	...
	async syncMerged(input: GitHubStatusSyncInput): Promise<GitHubStatusSyncResult> {
		try {
			...
			console.log(
				`[sync-service] Merge für Issue #${input.issueNumber} synchronisiert (closeIssue läuft im Orchestrator)`,
			);
			return {
				status: skipped ? 'skipped' : 'synced',
				labelsAdded: labels.added,
