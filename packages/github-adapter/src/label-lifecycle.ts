...
export const PHASE_LABELS: Record<Phase, { name: string; color: string; description: string }> = {
  ...
  REPO_SYNC:       { name: 'positron:repo-sync',     color: 'f9d0c4', description: 'Repository wird synchronisiert' },
  ISSUE_CONTEXT:   { name: 'positron:repo-sync',     color: 'f9d0c4', description: 'Issue-Kontext wird analysiert' },
  WEB_RESEARCH:    { name: 'positron:research',      color: 'f9d0c4', description: 'Web-Recherche läuft' },
  SPECIFY:         { name: 'positron:planning',      color: 'f9d0c4', description: 'Spezifikation wird erstellt' },
  CLARIFY_OPTIONAL:{ name: 'positron:planning',      color: 'f9d0c4', description: 'Klarstellung erforderlich' },
  ...
  IMPLEMENT:       { name: 'positron:implementing',  color: '0075ca', description: 'Implementierung läuft' },
  ...
  COMMIT:          { name: 'positron:implementing',  color: '0075ca', description: 'Änderungen werden committed' },
  PR_CREATE:       { name: 'positron:implementing',  color: '0075ca', description: 'Pull Request wird erstellt' },
  ...
  FAILED_TRANSIENT:{ name: 'positron:failed',        color: 'd73a4a', description: 'Fehlgeschlagen (wiederholbar)' },
  FAILED_BLOCKED:  { name: 'positron:blocked',       color: 'e99695', description: 'Blockiert' },
  FAILED_UNSAFE:   { name: 'positron:failed-unsafe', color: 'b60205', description: 'Unsicher fehlgeschlagen' },
  BLOCKED_PUSH:    { name: 'positron:blocked-push',  color: 'e99695', description: 'Push blockiert' },
  BLOCKED_MERGE:   { name: 'positron:blocked-merge', color: 'e99695', description: 'Merge blockiert' },
  GATE_APPROVE:    { name: 'positron:gate-approve',  color: 'fbca04', description: 'Warte auf Genehmigung' },
  GATE_REVISE:     { name: 'positron:gate-revise',   color: 'fbca04', description: 'Revision angefordert' },
  RESUME_PENDING:  { name: 'positron:resume-pending',color: 'fbca04', description: 'Fortsetzung ausstehend' },
...
export const LABEL_LIFECYCLE: Record<string, PhaseLabels> = {
  ...
  DONE: { add: ['positron:done'], remove: ['positron:running', 'positron:blocked', 'positron:failed', 'positron:implementing', 'positron:testing', 'positron:planning', 'positron:research', 'positron:repo-sync', 'positron:queued', 'positron:merging'] },
  FAILED: { add: ['positron:failed'], remove: ['positron:running', 'positron:implementing', 'positron:testing'] },
  FAILED_TRANSIENT: { add: ['positron:failed'], remove: ['positron:running'] },
  FAILED_BLOCKED: { add: ['positron:blocked'], remove: ['positron:running', 'positron:implementing', 'positron:testing'] },
