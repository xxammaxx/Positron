...
export function renderEvidenceSection(evidence: EvidenceItem[], _runId: string): string {
  ...
  const rows = evidence.map(e => {
    const icon = e.status === 'pass' ? '✅' : e.status === 'fail' ? '❌' : e.status === 'blocked' ? '🚫' : '⏭️';
 ...
 */
export function renderSyncPrCreated(runId: string, prNumber?: number, prUrl?: string, branchName?: string, issueNumber?: number): string {
 ...
 */
export function renderSyncMerged(runId: string, prNumber?: number, prUrl?: string, branchOrSha?: string): string {
