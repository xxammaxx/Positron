...
function makeIssue(overrides: Partial<GitHubIssueSummary> & { number: number }): GitHubIssueSummary {
  return {
    id: overrides.number,
...
function makeRepo(overrides: Partial<GitHubRepositorySummary> & { owner: string; name: string }): GitHubRepositorySummary {
  return {
    id: overrides.id ?? 1,
...
describe('FakeGitHubAdapter', () => {
  ...
  it('listPullRequests filtert nach state', async () => {
    await adapter.createPullRequest({
      owner: 'owner', repo: 'repo',
      title: 'Open PR', head: 'feature', base: 'main',
    });
  ...
  it('addIssueLabels und removeIssueLabel', async () => {
    adapter.addIssue(makeIssue({ number: 1 }));
    await adapter.addIssueLabels({ owner: 'owner', repo: 'repo', issueNumber: 1 }, ['positron:running']);
    await adapter.removeIssueLabel({ owner: 'owner', repo: 'repo', issueNumber: 1 }, 'positron:running');
