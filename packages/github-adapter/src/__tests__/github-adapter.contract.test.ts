...
describe('FakeGitHubAdapter implements GitHubAdapter contract', () => {
	...
	it('listOpenIssues() supports labels filter option', async () => {
		...
		const withFeature = await adapter2.listOpenIssues('test-owner', 'test-repo', {
			labels: ['feature'],
		});
	...
	it('createIssueComment() returns comment result with stable shape', async () => {
		const result: GitHubCommentResult = await adapter.createIssueComment(ref, 'Test comment');
	...
	it('addIssueLabels() resolves without error', async () => {
		await expect(adapter.addIssueLabels(ref, ['positron:running'])).resolves.toBeUndefined();
	});

	it('removeIssueLabel() resolves without error', async () => {
		await adapter.addIssueLabels(ref, ['positron:running']);
		await expect(adapter.removeIssueLabel(ref, 'positron:running')).resolves.toBeUndefined();
	});

	it('claimIssue() returns claim result with stable shape', async () => {
		const result: GitHubIssueClaimResult = await adapter.claimIssue(ref, claimOpts);

		expect(result).toHaveProperty('status');
		expect(['claimed', 'not_ready', 'already_claimed']).toContain(result.status);
	});
	...
	it('listPullRequestFiles() returns file list with stable shape', async () => {
		const files: GitHubPRFile[] = await adapter.listPullRequestFiles('test-owner', 'test-repo', 1);
	...
	it('getPullRequest() returns PR by number', async () => {
		...
		const fetched = await adapter.getPullRequest('test-owner', 'test-repo', created.number);
	...
	it('closeIssue() resolves without error for existing issue', async () => {
		...
		await expect(adapter2.closeIssue('test-owner', 'test-repo', 99)).resolves.toBeUndefined();
	});

	it('closeIssue() throws for non-existent issue', async () => {
		await expect(adapter.closeIssue('test-owner', 'test-repo', 99999)).rejects.toThrow();
	});
});
