...
describe('renderEvidenceSection', () => {
	...
	test('renders table for non-empty evidence', () => {
		const evidence: EvidenceItem[] = [{ kind: 'test', status: 'pass', summary: 'Tests passed' }];
	...
	test('renders fail status icon', () => {
		const evidence: EvidenceItem[] = [{ kind: 'test', status: 'fail', summary: 'Tests failed' }];
		const result = renderEvidenceSection(evidence, runId);
		expect(result).toContain('❌');
	});

	test('renders blocked status icon', () => {
		const evidence: EvidenceItem[] = [{ kind: 'gate', status: 'blocked', summary: 'Gate blocked' }];
		const result = renderEvidenceSection(evidence, runId);
		expect(result).toContain('🚫');
	});
