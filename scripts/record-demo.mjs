...
async function captureScene(page, name, url, waitMs = 2000) {
  try {
    ...
    try {
      await page.screenshot({ path: path.join(OUT_DIR, `scene-${name}-error.png`), fullPage: true });
    } catch {}
    return false;
  }
}
  ...
  try {
    ...
    try {
      ...
      if (data.runs && data.runs.length > 0) {
        ...
        await page.goto(`${BASE_URL}/runs/${runId}`, { waitUntil: 'domcontentloaded', timeout: 30000 });
        await page.waitForTimeout(3000);
        await page.screenshot({ path: path.join(OUT_DIR, 'scene-05-run-detail.png'), fullPage: true });
