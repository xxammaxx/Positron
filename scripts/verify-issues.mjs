...
function verifyIssue(issueNumber) {
  ...
  // Issue-Daten laden
  const issueJson = gh(
    `issue view ${issueNumber} --json number,title,body,state,labels,closedAt`
  );
  if (!issueJson) {
  ...
  const commitMatches = execSync(
    `git log --all --oneline --grep="#${issueNumber}"`,
  ...
  const keywords = criteria
    .flatMap((c) =>
      c.text
        ...
        .replace(/[^a-z0-9\s]/g, '')
        .split(/\s+/)
        .filter((w) => w.length > 3 && !['dass', 'nach', 'ohne', 'wird', 'eine', 'einen', 'einer', 'eines', 'diese', 'dieser', 'dieses', 'durch', 'über', 'unter', 'davon', 'daran', 'darin', 'dabei', 'damit', 'dafür', 'dagegen', 'sowie', 'oder', 'aber', 'auch', 'noch', 'schon', 'erst', 'sehr', 'alle', 'zwei', 'drei'].includes(w))
    );
  ...
  for (const kw of uniqueKeywords) {
    try {
      ...
      try {
        grepResult = execSync(
          `rg -l "${kw}" apps/ packages/ 2>&1`,
          { encoding: 'utf8', maxBuffer: 1024 * 1024, timeout: 5000 }
        ).trim();
      } catch {
        try {
          grepResult = execSync(
            `grep -rl "${kw}" apps/ packages/ 2>&1`,
            { encoding: 'utf8', maxBuffer: 1024 * 1024, timeout: 5000 }
          ).trim();
        } catch {
          grepResult = '';
