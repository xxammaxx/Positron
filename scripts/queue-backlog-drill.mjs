...
async function main() {
	...
	for (const arg of args) {
		if (arg.startsWith("--jobs=")) {
			...
			console.log(
				"  --jobs=N   Number of drill jobs (default: 60, min: 55, max: 75)",
			);
			process.exit(0);
		}
	}
	...
	const beforeRedis =
		getMetricValue(metricText, "positron_queue_redis_up") ?? 0;
	const beforeWorker =
		getMetricValue(metricText, "positron_queue_worker_up") ?? 0;

	console.log(`  Drill queue waiting:   ${BLUE}${beforeBacklog}${RESET}`);
	console.log(
		`  Pipeline queue waiting: ${BLUE}${beforePipelineBacklog}${RESET}`,
	);
	...
	if (beforeBacklog > 0) {
		log(
			"WARN",
			`Drill queue has ${beforeBacklog} leftover jobs — cleaning up first`,
		);
	}
	console.log("");
	...
	console.log(
		"  Waiting for server collect cycle (30s) + Prometheus scrape...",
	);
	...
	try {
		...
		console.log(
			`  Pipeline queue (metrics): ${BLUE}${afterPipelineBacklog}${RESET}`,
		);
		...
		try {
			...
			if (data?.data?.alerts) {
				...
				if (drillAlerts.length > 0) {
					log(
						"INFO",
						`QueueBacklogCriticalDrill ALERT: ${drillAlerts[0].state}`,
					);
					console.log(
						`      Alert state: ${BLUE}${drillAlerts[0].state}${RESET}`,
					);
					console.log(
					...
					log(
						"WARN",
						"QueueBacklogCriticalDrill NOT firing yet (waiting for Prometheus scrape)",
					);
					console.log(
						`      Note: Alert has 30s "for" — may need another scrape cycle.`,
					);
				}
				...
				if (productionAlerts.length > 0) {
					...
					console.log(
						`      This may be normal if the pipeline queue has >50 jobs.`,
					);
				} else {
	...
	if (mockOk) {
		try {
			...
			console.log(
				`  Drill alerts in mock: ${BLUE}${drillWebhooks.length}${RESET}`,
			);
